# Imports
- There are two kinds of imports in ghoti
...
- The standard file extension for ghoti files is `.gh`
...
import "node.gh"; // Illegal, file imports always require alias
pub import "node.gh" as node; // Public file import with alias
```
...
pub import "containers/array_list.gh" as array_list;
pub const ArrayList := array_list::ArrayList;
...
```

- `io/io.gh` declares a public function `println` which can be accessed via:
```ghoti
std::io::println(...);
```

- `containers/array_list.gh` declares a public function `ArrayList` that returns a generically-typed struct
```ghoti
