# Match
- In addition to `if` control flow, ghoti has match expressions
...
- Match expressions must always be exhaustive
    - To catch all arms that were not explicitly listed, you can use a catch-all branch where the LHS of the branch is the underscore `_` token
        - This branch does not support capturing as the value is already available in the scope
        - This branch may come anywhere in the match arm list
