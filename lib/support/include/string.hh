...
template <typename T>
concept StdStringLike = !std::same_as<T, const byte*> && StringLike<T>;
...
[[nodiscard]] constexpr auto trim_left(
    std::string_view str, Pred pred = [](byte b) -> bool { return std::isspace(b); }) noexcept
    -> std::string_view {
...
[[nodiscard]] constexpr auto trim_left(std::string_view str) noexcept -> std::string_view {
    return trim_left(str, [](byte b) -> bool { return std::isspace(b); });
}
...
[[nodiscard]] constexpr auto trim_right(std::string_view str) noexcept -> std::string_view {
    return trim_right(str, [](byte b) -> bool { return std::isspace(b); });
}
...
[[nodiscard]] constexpr auto trim(std::string_view str) noexcept -> std::string_view {
    return trim(str, [](byte b) -> bool { return std::isspace(b); });
}
...
[[nodiscard]] constexpr auto is_blank(std::string_view text) -> bool {
    return std::ranges::all_of(text, [](byte b) -> bool { return std::isspace(b); });
}
