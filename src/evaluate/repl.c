...
[[nodiscard]] Status repl_run(FileIO* io, char* stream_buffer, ArrayList* stream_receiver) {
    ...
    while (true) {
        ...
        array_list_clear_retaining_capacity(stream_receiver);
        TRY_IO_DO(fprintf(io->out, PROMPT), program_deinit(&program));
        TRY_IO_DO(fflush(io->out), program_deinit(&program));
        TRY_IO_DO(fflush(io->err), program_deinit(&program));

        TRY_DO(repl_read_chunked(io, stream_buffer, stream_receiver), program_deinit(&program));
...
repl_read_chunked(FileIO* io, char* stream_buffer, ArrayList* stream_receiver) {
    ...
    while (true) {
        if (!fgets(stream_buffer, BUF_SIZE, io->in)) {
            TRY_IO(fprintf(io->err, "Failed to read from input stream.\n"));
            TRY_IO(fprintf(io->out, "\n"));
            return READ_ERROR;
        }
        ...
        if (n > 0) {
            ...
            TRY(array_list_ensure_total_capacity(stream_receiver, needed));
