...
void check_errors(const ArrayList*         actual_errors,
                  ...
                  bool                     print_anyways) {
    MutSlice error;
    if (actual_errors->length == 0 && expected_errors.size() == 0) { return; }
...
void test_type_expression(const Expression* expression,
    ...
    const auto* type_expr = (const TypeExpression*)expression;
...
void test_decl_statement(const Statement*  stmt,
    ...
    const auto* decl_stmt = (const DeclStatement*)stmt;
    test_type_expression((const Expression*)decl_stmt->type,
                         expect_nullable,
                         expect_primitive,
                         expected_tag,
...
template <typename T> void test_number_expression(const Expression* expression, T expected_value) {
    if constexpr (std::is_same_v<T, double>) {
        const auto* f = (const FloatLiteralExpression*)expression;
        REQUIRE(f->value == expected_value);
    } else if constexpr (std::is_same_v<T, int64_t>) {
        const auto* i = (const IntegerLiteralExpression*)expression;
        REQUIRE(i->value == expected_value);
    } else if constexpr (std::is_same_v<T, uint64_t>) {
        const auto* i = (const UnsignedIntegerLiteralExpression*)expression;
        REQUIRE(i->value == expected_value);
    } else {
        REQUIRE(false);
    }
...
template <typename T> void test_number_expression(const char* input, T expected_value) {
    ...
    const auto* expr = (const ExpressionStatement*)stmt;
...
void test_bool_expression(const Expression* expression, bool expected_value) {
    const auto* boolean = (const BoolLiteralExpression*)expression;
...
void test_string_expression(const Expression* expression, std::string expected_string_literal) {
    const auto* string = (const StringLiteralExpression*)expression;
...
void test_identifier_expression(const Expression* expression,
    ...
    const auto* ident = (const IdentifierExpression*)expression;
